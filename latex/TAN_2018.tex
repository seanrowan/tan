...
\begin{proof}
By straightforward extension of the proof in Goodfellow et al. (2014), the following result is achieved on convergence of $\min_{G} \max_{D} V_s(G, D)$. 
\begin{equation}
p_s(\bm{x},\bm{z}) = G_x(\bm{x}|\bm{z};\theta_{gx}^*)p_s(\bm{z}).
\end{equation}
Similarly, by straightforward extension of the proof in Dumoulin et al. (2016), the following result is achieved on convergence of $\min_{G} \max_{D} V_t(G, D)$.
\begin{equation}
